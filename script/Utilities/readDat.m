...
% READAT: Reads binary MODFLOW heads or drawdown output into a struct, H
%
% USAGE:
...
%   Prepend filename with a "-" (minus) to get less verbose screen output.
...
if exist('userPeriods','var') && ~isempty(userPeriods) && userPeriods(1)<1
...
if exist('userPeriod','var') && ~isempty(userPeriod) && userPeriod(1)<=0,
...
for i=1:length(userPeriod), Select([H.per]  ==userPeriod(i),UPCOL)=i; end
...
for i=1:length(IPT)  % indices of required records in infile
   ...
   [H(iROut).tstp,...
    ...
    if verbose
        ...
        if rem(iROut,  50)==0, fprintf('.'); end
        if rem(iROut,2500)==0, fprintf('%d\n',iROut); end
    end
    if iL==LASTLAY
        iROut=iROut+1;
    end
