...
class SpacedDiffusionForm2(nn.Module):
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps, cond=None):
        # print(x_t)
        # print(eps)
        # assert x_t.shape == eps.shape # 这里被zx注释掉了，不然会报错int没有shape操作
        x_hat = 0 if cond is None else cond
        # assert x_hat.shape == x_t.shape # 这里被zx注释掉了
        return (
    ...
    def forward(   # 这里是spaced加速之后，但是不选择DDIM的推理过程。大致的步骤和ddpm_form2中的sample函数相同
        self,
        x_t,
        cond=None,
        z_vae=None,
    ...
    def get_ddim_mean_cov(  # 这个函数用于DDIM的推理过程
        self,
        x,
        t,
        clip_denoised=True,
    ...
    # 如果选择了DDIM的推理的话，会走这里。  大致的步骤和ddpm_form2中的sample函数相同
    def ddim_sample(
        self, x_t, cond=None, z_vae=None, checkpoints=[], eta=0.0, guidance_weight=0.0  # eta=0.0时是DDIM，可以去看前面DDIM的原始论文。
    ):
        # The sampling process goes here!
        x = x_t
        B, *_ = x_t.shape
        ...
        for idx, t in enumerate(reversed(range(0, num_steps))):
            ...
            assert z.shape == x_t.shape
            (post_mean, post_variance,) = self.get_ddim_mean_cov(  # 前面定义了这个函数
                x,
                t,
                cond=cond,
                z_vae=z_vae,
                eta=eta,  # DDIM会多一个η参数，用来控制是否是DDIM
                guidance_weight=guidance_weight,
            )
            nonzero_mask = (
                torch.tensor(t != 0, device=x.device)
